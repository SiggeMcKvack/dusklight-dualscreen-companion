...
void handleXYTap(int xy) {
    ...
    if (companionWolf()) {
        const char* action =
            (md != NULL && !touchUseBlocked()) ? ga::cast(md)->getActionTextXY(xy) : NULL;
    ...
    if (touchUseBlocked() || !ga::cast(md)->isItemUsable(xy)) {
        queueSound(Z2SE_SYS_ERROR, HAPTIC_DENY);
        s_denyFlash[xy] = DENY_FLASH_FRAMES;
        return;
    }
...
void handleSlotTap(int which) {
    ...
    dMeter2Draw_c* md = meterDraw();
    if (md == NULL || touchUseBlocked() || !ga::cast(md)->isItemUsable(2 + which)) {
        queueSound(Z2SE_SYS_ERROR, HAPTIC_DENY);
        s_denyFlash[2 + which] = DENY_FLASH_FRAMES;
        return;
    }
