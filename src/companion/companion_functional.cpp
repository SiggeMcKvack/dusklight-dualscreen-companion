...
bool drawLeftVesselBox(dMeter2Draw_c* md, f32 x1, f32 y0, f32 y1) {
    ...
    f32 savedScale[2];
    ga::cast(md)->pushVesselStateForCompanion(savedAlpha, savedX, savedY, savedScale);
    drawPaneComposite(ga::cast(md)->getLightDropPane(), 4.0f, y0 + 4.0f, x1 - 8.0f,
        y1 - y0 - LABEL_H - 8.0f);
    ga::cast(md)->popVesselStateForCompanion(savedAlpha, savedX, savedY, savedScale);
    ...
    for (int i = 0; i < 2; i++) {
        ...
        if (showItem) {
            ...
            drawItemIcon(ICON_SLOT_FN1 + i, item, px0 + (pside - icon) * 0.5f,
                ...
                md != NULL && ga::cast(md)->isItemUsable(2 + i) ? (u8)0xFF : (u8)128);
...
void drawFunctionalZCorner(dMeter2Draw_c* md, f32 lx0, f32 lx1, f32 by0, f32 by1) {
    ...
    J2DPane* midna = md != NULL ? ga::cast(md)->getMidnaButtonPaneRaw() : NULL;
...
void drawTransformCorner(f32 x0, f32 y0, f32 x1, f32 y1) {
    ...
    const bool enabled = unlocked && !anyMenuOpen() && alink != NULL &&
        ga::cast(alink)->checkQuickTransformOK();
    ...
    for (const Entry& e : entries) {
        ...
        if (itemNo != dItemNo_NONE_e) {
            ...
            drawItemIcon(ICON_SLOT_XITEM + e.xy, itemNo, pbx + (psz - picon) * 0.5f,
                ...
                ga::cast(md)->isItemUsable(e.xy) ? (u8)0xFF : (u8)128);
        ...
        const char* action = (!menuOpen && wolf) ? ga::cast(md)->getActionTextXY(e.xy) : NULL;
