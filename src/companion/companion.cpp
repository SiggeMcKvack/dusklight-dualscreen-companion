...
bool drawMeterBar(f32 x0, f32 x1, f32 cy, f32 barH, bool allowOil) {
    ...
    const bool oxygen = md != NULL && ga::cast(md)->isOxygenActive() && dComIfGp_getMaxOxygen() > 0;
...
void tickGaugeWarning() {
    ...
    // recovering a margin above it.
    {
        ...
        const bool oxActive =
            mdw != NULL && ga::cast(mdw)->isOxygenActive() && dComIfGp_getMaxOxygen() > 0;
        const f32 oxF =
...
bool warpPortalsShown() {
    ...
    if (mw == NULL || ga::cast(mw)->getMenuFmap() == NULL) {
        return false;
    }
    return ga::cast(ga::cast(mw)->getMenuFmap())->isWarpMapMode();
}
...
void drawSpecialPanel(f32 w, f32 h) {
    ...
    dMeterSub_c* sub = ga::cast(meter)->getSubContentsDlst();
    dMeterString_c* subSub = ga::cast(meter)->getSubSubContentsDlst();
...
void drawDashboard(float w, float h) {
    ...
    if (dMeter2Draw_c* mdAlpha = meterDraw()) {
        ga::cast(mdAlpha)->forceCompanionAlpha();
    }
    loadCollectIcons();
