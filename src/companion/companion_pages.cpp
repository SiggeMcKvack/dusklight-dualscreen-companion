...
void miniMapApplyPan(dMap_c* map, f32 drawW, f32 drawH) {
    ...
    if ((s_mapPanX != 0.0f || s_mapPanY != 0.0f) && map->getTexelPerCm() > 0.0f) {
        const f32 worldW = (f32)ga::cast(map)->getTexSizeX() / map->getTexelPerCm() * s_mapRenderScale;
    ...
    f32 loX = dMpath_c::getMinX() - ga::cast(map)->getCenterX();
    f32 hiX = dMpath_c::getMaxX() - ga::cast(map)->getCenterX();
...
void drawMiniMapContent(f32 x0, f32 y0, f32 x1, f32 y1) {
    ...
    dMap_c* map = meterMap != NULL ? ga::cast(meterMap)->getDMap() : NULL;
